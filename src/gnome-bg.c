...
/* Thumbnail utilities */
static GdkPixbuf *create_thumbnail_for_filename (const char            *filename);
			 ...
			 int	     height)
{
	...
	g_return_val_if_fail (bg != NULL, NULL);
	g_return_val_if_fail (gdk_surface != NULL, NULL);
			 ...
			 int		       *height)
{
	...
	thumb = create_thumbnail_for_filename (filename);
	if (thumb) {
			   ...
			   int                    dest_height)
{
	...
	if (bg->placement != G_DESKTOP_BACKGROUND_STYLE_NONE) {
		thumb = create_img_thumbnail (bg, surface, dest_width, dest_height, -1);
...
static GdkPixbuf *
get_as_thumbnail (GnomeBG *bg, const char *filename)
{
	const FileCacheEntry *ent;
	if ((ent = file_cache_lookup (bg, THUMBNAIL, filename))) {
		...
		GdkPixbuf *thumb = create_thumbnail_for_filename (filename);
		 ...
		 int	    dest_height)
{
	...
	if (get_thumb_annotations (thumb, &o_width, &o_height)		||
		...
		int scr_height = gdk_surface_get_height (surface);
		int scr_width = gdk_surface_get_width (surface);
		      ...
		      int                           frame_num)
{
	if (bg->filename) {
		...
		thumb = get_as_thumbnail (bg, bg->filename);
		...
		if (thumb) {
			...
			if (show) {
				...
				if (is_fixed) {
					...
					GdkPixbuf *p1, *p2;
					p1 = get_as_thumbnail (bg, file1);
					p2 = get_as_thumbnail (bg, file2);
...
/* Thumbnail utilities */
static GdkPixbuf *
create_thumbnail_for_filename (const char *filename)
{
	time_t mtime;
				 ...
				 int				 frame_num)
{
	...
	if (bg->placement != G_DESKTOP_BACKGROUND_STYLE_NONE) {
		thumb = create_img_thumbnail (bg, surface, dest_width, dest_height, frame_num + skipped);
