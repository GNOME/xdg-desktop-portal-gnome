...
global_shortcuts_session_close (Session *session)
{
  ...
  org_gnome_shell_call_unbind_shortcuts (shell, session_get_id (session), NULL, shell_unbind_shortcuts_done, NULL);
}
...
global_shortcuts_session_class_init (GlobalShortcutsSessionClass *klass)
{
  ...
  gobject_class->finalize = global_shortcuts_session_finalize;
...
static char *
rewrite_trigger (const char *trigger)
{
  char **pieces;
  struct {
    const char *from;
...
static GVariant *
rewrite_for_settings (GVariant *input)
{
  ...
  while (g_variant_iter_next (&iter, "(s@a{sv})", &id, &v))
    {
      ...
      if (preferred_trigger)
        {
          rewritten = rewrite_trigger (preferred_trigger);
          g_variant_builder_add (&builder, "{sv}", "preferred_trigger", g_variant_new_string (rewritten));
        }
...
static GVariant *
rewrite_for_response (GVariant *input, GVariant *shortcuts)
{
  ...
  while (g_variant_iter_next (&iter, "(s@a{sv})", &id, &v))
    {
      ...
      rewritten = g_strjoinv (" or ", (char **)trigger_desc);
      ...
      if (description)
        g_variant_builder_add (&builder, "{sv}", "description", g_variant_new_string (description));
      if (trigger_desc)
        g_variant_builder_add (&builder, "{sv}", "trigger_description", g_variant_new_string (rewritten));
      g_variant_builder_close (&builder);
      g_variant_builder_close (&builder);

      g_free (rewritten);
      g_free (description);
      g_strfreev (trigger_desc);
      g_variant_unref (v);
      g_free (id);
    }
                           ...
                           gpointer      data)
{
  ...
  GVariant *rewritten = NULL;
  GError *error = NULL;

  if (!org_gnome_shell_call_bind_shortcuts_finish (proxy, &results, &changed, result, &error))
    {
  ...
  rewritten = rewrite_for_response (shortcuts, handle->shortcuts);

  if (changed)
    xdp_impl_global_shortcuts_emit_shortcuts_changed (global_shortcuts, handle->session_handle, rewritten);

out:
  if (strcmp (g_dbus_method_invocation_get_method_name (handle->invocation), "RebindShortcuts") == 0)
    {
      org_gnome_global_shortcuts_rebind_complete_rebind_shortcuts (global_shortcuts_rebind, handle->invocation);
    }
  else
    {
      g_variant_builder_init (&results_builder, G_VARIANT_TYPE_VARDICT);
      if (rewritten)
        g_variant_builder_add (&results_builder, "{sv}", "shortcuts", rewritten);
  ...
  g_clear_pointer (&rewritten, g_variant_unref);
                              ...
                              gpointer      data)
{
  ...
  org_gnome_shell_call_bind_shortcuts (shell, handle->session_handle, shortcuts, NULL, shell_bind_shortcuts_done, handle);
                       ...
                       GVariant               *arg_options)
{
  ...
  rewritten = rewrite_for_settings (arg_shortcuts);

  char *s = g_variant_print (rewritten, FALSE);
                           ...
                           gpointer      data)
{
  ...
  response = 0;
  rewritten = rewrite_for_response (shortcuts, NULL);
